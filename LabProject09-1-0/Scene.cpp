...
void CScene::CreateShaderResourceView(ID3D12Device* pd3dDevice, CTexture* pTexture, int nIndex, UINT nRootParameterStartIndex)
{
	...
	if (pShaderResource && !d3dGpuDescriptorHandle.ptr)
	{
		...
		if (pTexture->m_nRootParameters == 1)
			pTexture->SetRootParameterIndex(0, nRootParameterStartIndex);
		else
		{
			if (pTexture->m_nRootParameters == pTexture->m_nTextures) pTexture->SetRootParameterIndex(nIndex, nRootParameterStartIndex + nIndex);
		}
	}
}
...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12CommandQueue* pd3dCommandQueue, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	CreateCbvSrvDescriptorHeaps(pd3dDevice, nObjects + 1 + 1 + 1 + 20, 1 + 6 + 6 + 3 + 20); //Player(1), Skybox(6)
