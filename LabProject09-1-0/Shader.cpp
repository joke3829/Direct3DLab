...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12CommandQueue* pd3dCommandQueue, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
}
...
void CBillboardObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12CommandQueue* pd3dCommandQueue, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	CScene::CreateShaderResourceViews(pd3dDevice, pBillboardTexture, 0, 10);
...
void CDynamicCubeMappingShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12CommandQueue* pd3dCommandQueue, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	HRESULT hResult = pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, __uuidof(ID3D12Fence), (void**)&m_pd3dFence);
	m_nFenceValue = 0;

	m_nObjects = 2;
	m_ppObjects = new CGameObject*[m_nObjects];

	CreateShaderVariables(pd3dDevice, m_pd3dCommandList);
	...
	CMesh* pMeshIlluminated = new CSphereMeshIlluminated(pd3dDevice, m_pd3dCommandList, 100.0f, 20, 20);
	...
	{
		m_ppObjects[i] = new CDynamicCubeMappingObject(pd3dDevice, m_pd3dCommandList, m_nCubeMapSize, d3dDsvCPUDescriptorHandle, d3dRtvCPUDescriptorHandle, this);
	...
	::WaitForGpuComplete(pd3dCommandQueue, m_pd3dFence, ++m_nFenceValue, m_hFenceEvent);
}
...
void CDynamicCubeMappingShader::OnPreRender(ID3D12Device* pd3dDevice, ID3D12CommandQueue* pd3dCommandQueue, CScene* pScene)
{
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		::WaitForGpuComplete(pd3dCommandQueue, m_pd3dFence, nFenceValue + 1, m_hFenceEvent);
	}
}
